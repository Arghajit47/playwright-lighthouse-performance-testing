...
const BestPracticeChart = ({ data, allData, theme }) => {
  ...
  const [bestPracticeFilter, setBestPracticeFilter] = React.useState("All");

  const device_typeTypes = [...new Set(allData.map((d) => d.device_type))];

  useEffect(() => {
    applyFilters();
  }, [device_typeFilter, bestPracticeFilter, data]);
  ...
  const applyFilters = () => {
    ...
    if (bestPracticeFilter !== "All") {
      let [min, max] = bestPracticeFilter.split("-").map(Number);
      if (min > max) [min, max] = [max, min];
      filtered = filtered.filter(
        (d) => d.best_practice >= min && d.best_practice <= max
      );
    }
  ...
  useEffect(() => {
    ...
    const barData = testNames.map((test) => latestTests[test][0].best_practice);
    ...
    chartInstance.current = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          ...
          title: {
            display: true,
            text: "Best Practices for Last 3 Runs",
  ...
  const filters = (
    <FilterContainer
      ...
      selectedValue={bestPracticeFilter}
      onValueChange={setBestPracticeFilter}
      valueOptions={[
        { value: "All", label: "All" },
  ...
  return (
    <ChartContainer title="Best Practices for Last 3 Runs" filters={filters}>
      <canvas id="bestPracticeChart" ref={chartRef}></canvas>
    </ChartContainer>
  );
};
