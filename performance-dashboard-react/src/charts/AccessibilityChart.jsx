...
const AccessibilityChart = ({ data, allData, theme }) => {
  ...
  useEffect(() => {
    applyFilters();
  }, [device_typeFilter, accessibilityFilter, data]);
  ...
  const applyFilters = () => {
    ...
    if (accessibilityFilter !== "All") {
      let [min, max] = accessibilityFilter.split("-").map(Number);
      if (min > max) [min, max] = [max, min];
      filtered = filtered.filter(
        (d) => d.accessibility >= min && d.accessibility <= max
      );
    }
  ...
  const filters = (
    <FilterContainer
      ...
      selectedValue={accessibilityFilter}
